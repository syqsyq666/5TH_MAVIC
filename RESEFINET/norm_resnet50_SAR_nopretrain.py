...
def _load_local_pretrained(model, pth_path, device='cpu'):
    """从根目录本地权重加载，只加载形状匹配的参数（兼容 ResNeSt 与 ResNet 结构差异）。"""
...
def train(train_loader, unlabeled_train_loader, device_1, device_2,batch_size):
    # 使用本地权重，不下载：根目录 resnest50-528c19ca.pth（只加载形状匹配部分，兼容 ResNeSt/ResNet 差异）
    try:
    ...
    _load_local_pretrained(model_EO, os.path.join(BASE_DIR, 'resnest50-528c19ca.pth'), device_1)
    ...
    _load_local_pretrained(model_SAR, os.path.join(BASE_DIR, 'resnest50-528c19ca.pth'), device_2)
...
if __name__ == "__main__":
    ...
    train_loader, unlabeled_train_loader, train_dataset_size_EO, train_dataset_size_SAR = prepare_data_loaders(
        EO_file_pth, SAR_file_pth, batch_size=32, test_size=0.2, num_workers=num_workers, eo_transform=transform, sar_transform=transform
    )
